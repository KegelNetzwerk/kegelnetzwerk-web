...
export interface MemberCommentData {
  id: number;
  content: string;
  createdAt: string;
  authorName: string;
...
export default function MemberComments({
  ...
  const [comments, setComments] = useState<MemberCommentData[]>(initialComments);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      const comment: MemberCommentData = await res.json();
  ...
  async function handleDelete(commentId: number) {
    const res = await fetch(`/api/member-comments/${commentId}`, { method: 'DELETE' });
    if (res.ok) {
