...
interface PublicCommentsProps {
  ...
  readonly initialComments: PublicCommentData[];
...
export default function PublicComments({
  ...
  const [comments, setComments] = useState<PublicCommentData[]>(initialComments);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      const comment: PublicCommentData = await res.json();
  ...
  async function handleDelete(commentId: number) {
    const res = await fetch(`${apiPath}/${commentId}`, { method: 'DELETE' });
    if (res.ok) {
